...
def config_report() -> list[dict]:
    ...
    add(
        ...
        LLM_PROVIDER
        if LLM_PROVIDER in _LLM_PROVIDERS
        else f"{LLM_PROVIDER!r} (expected: {', '.join(_LLM_PROVIDERS)})",
    )
    add("OLLAMA_BASE_URL", "required" if _is_http(OLLAMA_BASE_URL) else "invalid", OLLAMA_BASE_URL)
    ...
    stt_ok = STT_MODEL.startswith("whisper-") and STT_MODEL.removeprefix("whisper-") in _STT_SIZES
    add(
        "STT_MODEL",
        "required" if stt_ok else "invalid",
        STT_MODEL if stt_ok else f"{STT_MODEL!r} (expected: whisper-<{'|'.join(_STT_SIZES)}>)",
    )
    ...
    add(
        ...
        MLX_MODE
        if MLX_MODE in _MLX_MODES
        else f"{MLX_MODE!r} (expected: auto|true|false; using auto)",
    )
    add("MLX_BASE_URL", "optional" if _is_http(MLX_BASE_URL) else "invalid", MLX_BASE_URL)
    add(
        "MLX_REQUEST_TIMEOUT",
        "invalid"
        if _invalid_float("MLX_REQUEST_TIMEOUT", 1.0) or MLX_REQUEST_TIMEOUT < 1.0
        else "optional",
        str(MLX_REQUEST_TIMEOUT),
    )
    ...
    add(
        "MAX_UPLOAD_MB",
        "invalid" if _invalid_int("MAX_UPLOAD_MB", 1) or MAX_UPLOAD_BYTES <= 0 else "optional",
        str(MAX_UPLOAD_BYTES // (1024 * 1024)),
    )
    add(
        "MEDIA_MAX_UPLOAD_MB",
        "invalid"
        if _invalid_int("MEDIA_MAX_UPLOAD_MB", 1) or MEDIA_MAX_UPLOAD_BYTES <= 0
        else "optional",
