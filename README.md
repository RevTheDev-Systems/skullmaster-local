...
- **Local tools (Phase 22)** — a safe, auditable tool layer (`calculator`, `days_between`, `convert`, `word_count`) at `GET /api/tools` / `POST /api/tools/run`; no code execution, strict validation. Opt in with the **🧮 Tools** toggle to let a research answer use one logged local tool before answering
